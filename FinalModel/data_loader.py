...
class Dataset():
    def __init__(self, dataset, path, num_classes1=10, num_classes2=15, num_classes3=20):
    ...
    def __call__(self, validation):
        ...
        for i in range(3):
            train_data, train_label = zip(*(data[i][0]))
            test_data, test_label = zip(*(data[i][1]))
            ...
            tmp_test = test_label.copy()
            one_hot(tmp_test, self.num_labels3)

            if i==0:
                one_hot(train_label, self.num_labels1)
                one_hot(test_label, self.num_labels1)
            elif i==1:
                one_hot(train_label, self.num_labels2)
                one_hot(test_label, self.num_labels2)
            elif i==2:
                one_hot(train_label, self.num_labels3)
                one_hot(test_label, self.num_labels3)

            print('train data length: %d' %(len(train_label)))
            print('test data length: %d' %(len(test_label)))

            dataset_train.append([train_data, train_label])
            dataset_test.append([test_data, test_label])

            test_all_data += test_data
            test_all_label += tmp_test

        print('Test Data: %d, %d' %(len(test_all_data), len(test_all_label)))
        return dataset_train, dataset_test, (test_all_data, test_all_label)
    ...
    def slice(self, data_list, validation):
        ...
        c_start = 0
        c_end = 0
        tmp = [[] for i in range(self.num_labels3)]
        ...
        for i in range(len(data_list)):
            ...
            if data_list[i][1] >= self.num_labels3:
                end = i
                break
        ...
        if c_end != end:
            ...
            c += 1
        assert c==self.num_labels3

        train1, test1 = [], []
        train2, test2 = [], []
        train3, test3 = [], []
        for i in range(self.num_labels3):
