...
class ResNet(object):
    def __init__(self, config, is_training = False):
        ...
        h = self.res_block(h, 16, 'layer1-'+str(0), True)
        for i in range(1,n):
        ...
        #h = tf.reduce_max(h, 1)
        y = self.fc(h, self.num_classes, 'fc-layer')
    ...
    def conv_bn_relu(self, x, num_out, scope):
        x_conv = self.conv(x, num_out, 1, scope)
        x_bn = self.batch_norm(x_conv, num_out, scope)
        x_relu = self.relu(x_bn)
        return x_relu

    def bn_relu_conv(self, x, num_out, stride, scope):
    ...
    def res_block(self, x, num_out, scope, first_block=False):
        num_in = int(x.shape[3])
        if num_out//num_in == 2:
            stride = 2
        else:
        ...
        with tf.variable_scope(scope, reuse = tf.AUTO_REUSE):
            if first_block is True:
                x_1 = self.conv(x, num_out, 1, scope+'-1')
            else:
                x_1 = self.bn_relu_conv(x, num_out, stride, scope+'-1')

            x_2 = self.bn_relu_conv(x_1, num_out, 1, scope+'-2')
