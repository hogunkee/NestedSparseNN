...
class SparseResNet(object):
    ...
    def first_layer(self, x):
        with tf.variable_scope('input'):
            ...
            filter1 = tf.get_variable(name='conv', shape=[3,3,3,12], initializer = c_init)
            filter2 = tf.get_variable(name='conv', shape=[3,3,3,4], initializer = c_init)

            out1 = tf.nn.conv2d(x, filter1, strides = [1,1,1,1], padding='SAME')
            out2 = tf.nn.conv2d(x, filter2, strides = [1,1,1,1], padding='SAME')
    ...
    def lv1conv(self, x, dim, stride, scope):
        with tf.variable_scope(scope):
            ...
            c_init = tf.random_uniform_initializer(-n, n)
            filter = tf.get_variable(name='conv', shape=[3,3,int(x.shape[3]),dim], 
                    initializer = c_init)

            out = tf.nn.conv2d(x, filter, strides = [1,stride,stride,1], padding='SAME')
        return out
    ...
    def lv2conv(self, x1, x2, dim1, dim2, stride, scope):
        with tf.variable_scope(scope):
            ...
            filter1 = tf.get_variable(name='conv1', shape=[3,3,int(x2.shape[3]),dim1], 
                    initializer = c_init)
            filter2 = tf.get_variable(name='conv2', shape=[3,3,int(concat_x.shape[3]),dim2], 
                    initializer = c_init)

            out1 = tf.nn.conv2d(x2, filter1, strides = [1,stride,stride,1], padding='SAME')
            out2 = tf.nn.conv2d(concat_x, filter2, strides = [1,stride,stride,1], padding='SAME')
    ...
    def fc(self, x, dim, scope):
        with tf.variable_scope(scope):
            if not self.is_training:
                tf.get_variable_scope().reuse_variables()
            fc_w = tf.get_varisble(shape=[int(x.shape[3]), dim],
                    initializer=tf.uniform_unit_scaling_initializer(factor=1.0))
            fc_b = tf.get_variable(shape=[dim], initializer=tf.zeros_initializer())
